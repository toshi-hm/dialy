...
| Server Actions | React Server Actions | 19 | データ変更 |
| ORM | Prisma | 7 | データベースアクセス |
| データベース | PostgreSQL (Supabase) | - | データ永続化 |
| DBアダプター | @prisma/adapter-pg | 7 | Prisma PostgreSQLドライバ |
...
│  │  - LocalStorageDiaryRepository (Client)          │   │
│  │  - SupabaseDiaryRepository (Server)              │   │
│  │  - repository-factory.ts (切り替え)               │   │
│  └─────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
```
...
│   │       ├── diary.ts          # 日記関連のアクション（CRUD）
│   │       ├── types.ts          # ActionResult型定義
│   │       └── index.ts          # 再エクスポート
│   │
...
│   │   │   ├── local-storage-diary-repository.ts  # クライアント版: LocalStorage実装
│   │   │   ├── supabase-diary-repository.ts       # サーバー版: Supabase (PostgREST) 実装
│   │   │   ├── supabase-client.ts                 # Supabaseクライアント初期化
│   │   │   └── repository-factory.ts              # リポジトリファクトリ
│   │   │
...
- **Repositories**:
  ...
  - `SupabaseDiaryRepository` (Phase 2.1以降): Supabase (PostgREST) 実装
...
└──────┬──────────────┘
       ...
       ↓
┌──────────────────────────┐
│ SupabaseDiaryRepository  │  (Infrastructure)
│  - PostgreSQL (Supabase) に保存 │
└──────────────────────────┘
```
