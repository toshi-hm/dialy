...
| DBアダプター | @prisma/adapter-better-sqlite3 | 7 | Prisma SQLiteドライバ |
...
**Phase 2.1で実装済み**:
- **データベース**: Prisma 7 + SQLite（`@prisma/adapter-better-sqlite3`）
- **Server Actions**: `src/app/actions/diary.ts` でサーバーサイドCRUD
- **リポジトリ切り替え**: クライアント→LocalStorage、サーバー→Prisma
...
│   │   │   ├── prisma-diary-repository.ts         # サーバー版: Prisma実装
